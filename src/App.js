...
class App extends Component {
  ...
  handleImgClick = id => {
    ...
    if (id === randomObject.id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomObject: imagesList[Math.floor(Math.random() * imagesList.length)],
      }))
    } else {
      clearInterval(this.timer)
  ...
  render() {
    const {tabId, score, time, randomObject, gameStatus} = this.state
    const filteredImagesList = imagesList.filter(
      image => image.category === tabId,
    )
    ...
    return (
      <div>
        ...
        {gameStatus === 'playing' && (
          <div>
            <div>
              <img src={randomObject.imageUrl} alt="match" />
            </div>
            <div>
              <ul className="tab-items">
                {tabsList.map(tab => (
                  <li key={tab.tabId}>
                    <button onClick={() => this.handleTabClick(tab.tabId)}>
                      {tab.displayText}
                    </button>
                  </li>
                ))}
            ...
            <ul className="thumbnail-items">
              {filteredImagesList.map(image => (
                <li key={image.id}>
                  <button onClick={() => this.handleImgClick(image.id)}>
